...
DIRECT_CODE_SYSTEM_PROMPT = '''You are an expert algorithm designer specializing in bin packing optimization.

CRITICAL INSTRUCTIONS - You MUST follow this format exactly:

Output only one Python code block containing exactly one function:
...
class ThoughtsAugmentedLLM(LLM):
    ...
    def __init__(self, samples_per_prompt: int, api_key: str = "", use_thoughts: bool = True):
        super().__init__(samples_per_prompt)
        self._api_key = api_key or _API_KEY
        self._system_prompt = (
            THOUGHTS_AUGMENTED_SYSTEM_PROMPT if use_thoughts
            else DIRECT_CODE_SYSTEM_PROMPT
        )
